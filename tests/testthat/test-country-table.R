...
test_that("country_table errors when data is not a data frame", {
  expect_error(country_table("not a data frame", "Brazil"), "`data` must be a data frame")
})

test_that("country_table errors on a numerical, non character country", {
  expect_error(country_table(cases_year, 5), "`selected_country` must be a single character string")
})

test_that("country_table errors when the country is not found", {
  expect_error(country_table(cases_year, "GreeNl@nd"), "not found in the data's `country` column")
})
